...
const Cart = () => {
  ...
  let {reduce} = useContext(CartContext)
                  ...
                  {cartItems?  React.Children.toArray(
                    cartItems.map((item) => (
                      <div className='cart_list'>
                        ...
                        <p>Size: {item.size}</p>
                        <div className='quantity_selector'> Quantity: <div className='quantity_wrapper'><button onClick={event => reduce(event, item.size, item.id, item.quantity, item.price, item.description, item.src, item.alt)}><MdRemove/></button><p>{item.quantity}</p><button onClick={event => increase(event, item.size, item.id, item.quantity, item.price, item.description, item.src, item.alt)}><MdAdd/></button></div> </div>
